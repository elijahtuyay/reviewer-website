            ...
            return (
              <div key={exam.id}>
                <div className="flex items-baseline justify-between">
                  <Link href={`/${exam.id}`} className="text-lg font-semibold text-foreground hover:underline">
                    {exam.label}
                  </Link>
                  {exam.available ? (
                ...
                <p className="mt-1 text-sm text-muted">
                  {exam.available
                    ? `${totalQuestions} questions across ${exam.sections.length} independently-timed sections, matching the real exam.`
                    : exam.description}
                </p>
